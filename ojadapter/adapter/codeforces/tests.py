...
class TestAdapterCodeforces(TestCase):
    ...
    def test_05_parse_problem(self):
        ...
        problem = self.adapter.parse_problem(html)
        self.assertEqual(problem.title, 'Прокат велосипедов')
        # 这一类的数据输入 input 换行因为多了 <br> 所以会粘在一起，加入了特殊处理
        html = request_text(self.adapter.get_problem_url('1005A'))
        problem = self.adapter.parse_problem(html)
        ...
        self.assertEqual(len(problem.output_samples), 4)
        self.assertEqual(problem.output_samples[0], '2\n3 4 ')
        # TODO: 100J 有 <sub>/<sup> 的上下标内容，需要保留
